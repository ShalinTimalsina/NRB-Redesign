...
# 1. Base template structure (derived from the site's standard)
def get_header_and_footer(source_file):
    with open(source_file, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Split the file at the main tag
    parts = re.split(r'<main[^>]*>', content)
    header_part = parts[0] + '<main id="Main-Content">\n'
    
    # Split the remainder at the end of main
    footer_part = '\n</main>\n' + re.split(r'</main>', parts[1])[-1]
    
    return header_part, footer_part

# We will extract header and footer from monetary-policy/index.html because we know it is perfectly styled.
# Note: Since regulation-and-supervision/index.html is depth 2 (like monetary policy), we can reuse it exactly.
header, footer = get_header_and_footer('pages/monetary-policy/index.html')

# Replace Monetary Policy specifics in header
header = header.replace('<title>Monetary Policy | Nepal Rastra Bank</title>', '<title>Regulation & Supervision | Nepal Rastra Bank</title>')
# We also need to fix the active nav state in header
header = header.replace("const m = a.href.match(/\\/pages\\/([^\\/]+)\\//);", "const m = a.href.match(/\\/pages\\/([^\\/]+)\\//);") # no change needed
...
with open('pages/regulation-and-supervision/index.html', 'w', encoding='utf-8') as f:
    f.write(header + hub_content + footer)

print("Hub page rebuilt successfully.")
