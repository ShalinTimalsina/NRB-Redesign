...
# We need to replace the entire <div class="relative group">...</div> for Statistics.
# Let's write a regex that safely replaces the whole block.
# Actually, it's easier to find the old block and replace it.
# The old block has: <!-- Column 1: Macroeconomic -->
# We can search for the entire group block containing Nav-Statistics.
...
<a class="px-2 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-[#138496] font-medium transition-colors rounded-lg mb-1" href="{depth}pages/statistics/monetary-forex/index.html">
<span class="block">Monetary & Forex Hub</span>
<span class="block text-xs text-slate-400 font-normal mt-0.5 leading-relaxed">Monetary operations and interest rates.</span>
</a>
<a class="px-2 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-[#138496] font-medium transition-colors rounded-lg" href="{depth}pages/forex-management/index.html">
<span class="block font-semibold text-[#138496]">Live Daily Exchange Rate &rarr;</span>
<span class="block text-xs text-slate-400 font-normal mt-0.5 leading-relaxed">Interactive foreign exchange dashboard.</span>
</a>
</div>
...
for root, _, files in os.walk(base_dir):
    for file in files:
        if file.endswith('.html'):
            ...
            # Find the start of Statistics nav block
            # It starts with: <div class="relative group">\s*<a[^>]*?id="Nav-Statistics"
            block_pattern = r'<div class="relative group">\s*<a[^>]*?id="Nav-Statistics".*?<!-- Column 4: Financial Inclusion -->.*?</div>\s*</div>\s*</div>\s*</div>'
            
            if re.search(block_pattern, content, flags=re.DOTALL | re.IGNORECASE):
                replacement = new_mega_menu_template.format(depth=depth)
                new_content = re.sub(block_pattern, replacement, content, flags=re.DOTALL | re.IGNORECASE)
...
print(f"Updated {count} HTML files with the Forex link in Mega Menu.")
