...
print("1. Fixing Monetary Policy Nav State")
count = 0
for root, dirs, files in os.walk('c:/Users/LOQ/Desktop/NRB Redesign'):
    for file in files:
        if file.endswith('.html'):
            ...
            orig = content
            # Fix the Monetary Policy nav state
            content = content.replace(
                'text-white border-b-2 border-white pb-0.5',
                'text-slate-100 transition hover:text-[#138496]'
            )
            ...
            if content != orig:
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(content)
                count += 1
print(f"Fixed active state in {count} files.")


print("2. Re-building Media & Speeches subpages to fix links and remove 'Verified' text")
...
# Use monetary-policy/circulars/index.html as template (which we just fixed)
header, footer = get_header_and_footer('c:/Users/LOQ/Desktop/NRB Redesign/pages/monetary-policy/circulars/index.html')
...
sections = [
    {
        ...
        'md_file': 'c:/Users/LOQ/Desktop/NRB Redesign/NRB Page md files/Media and Speeches/Media_Releases.md'
    },
    {
        'id': 'speeches',
        'title': 'Speeches',
        'desc': 'Official speeches delivered by the Governor of Nepal Rastra Bank.',
        'md_file': 'c:/Users/LOQ/Desktop/NRB Redesign/NRB Page md files/Media and Speeches/Governors_Speeches.md'
    },
    {
        'id': 'notices',
        'title': 'Notices',
        'desc': 'Official notices issued by the Office of the Governor.',
        'md_file': 'c:/Users/LOQ/Desktop/NRB Redesign/NRB Page md files/Media and Speeches/Notices_OFG.md'
    },
    {
        'id': 'bok-kpp',
        'title': 'BOK-KPP Study Reports',
        'desc': 'Joint study reports under the Bank of Korea – Knowledge Partnership Program.',
        'md_file': 'c:/Users/LOQ/Desktop/NRB Redesign/NRB Page md files/Media and Speeches/BOK_KPP_Study_Reports.md'
    }
]
...
def extract_list_items(md_path):
    ...
    for line in lines:
        ...
        if line.startswith('###') and line[4].isdigit():
            ...
            meta_text = line[2:].strip().replace('**', '')
            if meta_text.startswith('Verified'):
                continue # SKIP VERIFIED TEXT ENTIRELY
            elif meta_text.startswith('Source:') or meta_text.startswith('PDF Link:'):
                ...
                current_item['meta'].append(meta_text)
                
    if current_item: items.append(current_item)
    return items

for sec in sections:
    sec_header = header.replace('<title>Circulars | Monetary Policy | Nepal Rastra Bank</title>', f'<title>{sec["title"]} | Media & Speeches | Nepal Rastra Bank</title>')
...
"""
    ...
    out_dir = f'c:/Users/LOQ/Desktop/NRB Redesign/pages/media-speeches/{sec["id"]}'
    out_path = os.path.join(out_dir, 'index.html')
    
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(full_html)
        
    print(f"Re-scaffolded {out_path}")
